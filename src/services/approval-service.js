...
function _recordAudit(e) { if (_auditRecorder) try { _auditRecorder(e); } catch {} }
...
async function _withTransaction(db, fn) {
 ...
 */
async function createApprovalRequest(db, params) {
    try {
        ...
        logger.info('approval-service', 'approval_request_created', { org_id: params.org_id, user_id: params.requested_by_user_id, approval_id: id, action_key: params.action_key, mode: params.approval_mode || 'SINGLE' });
        _recordAudit({ event_type: 'APPROVAL_CREATED', org_id: params.org_id, actor_user_id: params.requested_by_user_id, action_key: params.action_key, approval_id: id, outcome: 'PENDING', detail: { mode: params.approval_mode, risk: params.risk_level } });
 ...
 */
async function listApprovalRequests(db, filters = {}) {
    try {
        const conditions = [];
        const params = [];
        ...
        const countRow = await db.prepare(`SELECT COUNT(*) as total FROM approval_requests ${where}`).get(...params);
 ...
 */
async function getApprovalRequest(db, id, orgId) {
    try {
 ...
 */
async function getApprovalRequestInternal(db, id) {
    try {
        const row = await db.prepare('SELECT * FROM approval_requests WHERE id = ?').get(id);
        if (!row) return { success: false, error: 'approval_request_not_found' };
        return { success: true, request: _parseRow(row) };
    } catch (err) {
        return { success: false, error: err.message };
...
async function approveApprovalRequest(db, id, params = {}) {
    return await _transitionToTerminal(db, id, APPROVAL_STATUS.APPROVED, params);
}
async function rejectApprovalRequest(db, id, params = {}) {
    return await _transitionToTerminal(db, id, APPROVAL_STATUS.REJECTED, params);
}
async function cancelApprovalRequest(db, id, params = {}) {
    return await _transitionToTerminal(db, id, APPROVAL_STATUS.CANCELLED, params);
}

async function summarizeApprovalRequests(db, orgId) {
    try {
...
// ------------------------------------------------------------
async function _transitionToTerminal(db, id, newStatus, params = {}) {
    try {
        ...
        const result = await _withTransaction(db, async () => {
            ...
            if (row.approval_mode === 'DUAL' && newStatus === APPROVAL_STATUS.APPROVED) {
                return await _handleDualApproval(db, row, actorUserId, reason, metadata, now);
            }
            ...
            if (updateResult.changes === 0) {
                ...
                const reread = await db.prepare('SELECT * FROM approval_requests WHERE id = ?').get(id);
            ...
            const updated = await db.prepare('SELECT * FROM approval_requests WHERE id = ?').get(id);
            return { success: true, request: _parseRow(updated) };
        });

        if (result && result.success && !result.idempotent && result.request) {
            const eventType = 'APPROVAL_' + newStatus;
            logger.info('approval-service', 'state_transition', { org_id: orgId || result.request.org_id, user_id: actorUserId, approval_id: id, from: 'PENDING', to: newStatus });
            _recordAudit({ event_type: eventType, org_id: orgId || result.request.org_id, actor_user_id: actorUserId, approval_id: id, outcome: newStatus, detail: { reason, mode: result.request.approval_mode } });
        }
 ...
 */
async function _handleDualApproval(db, row, actorUserId, reason, metadata, now) {
    const id = row.id;
    ...
    // Second approval
    const secondResult = await db.prepare(`
        UPDATE approval_requests
...
async function _getLastInsertId(db) {
    try {
...
module.exports = {
    ...
    APPROVAL_STATUS, VALID_TRANSITIONS, setAuditRecorder,
};

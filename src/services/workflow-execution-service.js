...
async function _lid(db) {
    try { const r = await db.prepare('SELECT last_insert_rowid() AS id').get(); return r ? r.id : null; }
    catch { return null; }
}
...
// Executor: only callable through governed path
let _ex = () => ({ success: true, result: { executed: true, engine: 'default' } });
let _governanceVerified = false;

function configureExecutor(fn) {
    if (process.env.NODE_ENV === "production") { logger.warn("execution-service", "configureExecutor blocked in production"); return; }
    if (typeof fn === 'function') _ex = fn;
    else _ex = () => ({ success: true, result: { executed: true, engine: 'default' } });
}

// PRIVATE: execute only after governance gate has cleared
async function _runGoverned(db, wid, p, ctx) {
    if (!_governanceVerified) {
        throw new Error('GOVERNANCE_BYPASS: _runGoverned called without governance clearance');
    }
    try { return _ex(db, wid, p, ctx); }
    catch (e) { return { success: false, error: 'executor_error: ' + e.message }; }
}

async function executeWorkflow(db, params = {}) {
    try {
        ...
        _governanceVerified = false;

        // MANDATORY: all execution goes through governance gate
        const gate = await enforceGovernance(db, {
            org_id: params.org_id,
            target_type: 'workflow',
            target_id: params.workflow_id,
        ...
        if (gate.status === GATE_STATUS.CLEAR) {
            _governanceVerified = true;
            const er = await _runGoverned(db, params.workflow_id, params.payload || {}, {
            ...
            const ins = await db.prepare(
                ...
                _js(params.payload), _js(er), _js({ gate_status: 'CLEAR' }), now, now);
            return {
                success: true,
                execution_id: ins.lastInsertRowid ? Number(ins.lastInsertRowid) : await _lid(db),
        ...
        if (gate.status === GATE_STATUS.PENDING) {
            const ins = await db.prepare(
                ...
                'BLOCKED_PENDING_APPROVAL', gate.approval_request_id,
                _js(params.payload),
                _js({ gate_status: 'PENDING', approval_request_id: gate.approval_request_id }),
                now, now);
            return {
                success: true,
                execution_id: ins.lastInsertRowid ? Number(ins.lastInsertRowid) : await _lid(db),
        ...
        // ERROR or DENIED — fail closed
        const ins = await db.prepare(
            'INSERT INTO workflow_executions (org_id, workflow_id, actor_user_id, execution_status, request_payload_json, result_payload_json, governance_snapshot_json, is_replay, created_at, updated_at) VALUES (?,?,?,?,?,?,?,0,?,?)'
        ).run(params.org_id, params.workflow_id, params.actor_user_id, 'BLOCKED_ERROR',
            _js(params.payload), _js({ error: gate.error }),
            _js({ gate_status: gate.status, error: gate.error }), now, now);
        return {
            success: false,
            execution_id: ins.lastInsertRowid ? Number(ins.lastInsertRowid) : await _lid(db),
            execution_status: 'BLOCKED_ERROR', error: gate.error,
        };
    } catch (e) {
...
async function replayApprovedExecution(db, params = {}) {
    try {
        if (!params.org_id) return { success: false, error: 'org_id_required' };
        ...
        // MANDATORY: verify approval through governance gate
        _resetGovernanceFlag();
        _governanceVerified = false;
        const gate = await enforceGovernance(db, {
            org_id: params.org_id,
            target_type: 'workflow',
            target_id: orig.workflow_id,
            action_type: 'replay',
            actor_user_id: params.actor_user_id,
            existing_approval_id: orig.approval_request_id,
        });
        assertGovernanceEnforced();
        ...
        if (gate.status === GATE_STATUS.APPROVED) {
            ...
            const er = await _runGoverned(db, orig.workflow_id, payload, {
                org_id: params.org_id, actor_user_id: params.actor_user_id,
            ...
            const ins = await db.prepare(
                ...
                _js({ gate_status: 'APPROVED', original_execution_id: orig.id, replayed_by: params.actor_user_id }),
                ik, params.actor_user_id, now, now, now);

            const rid = ins.lastInsertRowid ? Number(ins.lastInsertRowid) : await _lid(db);
            await db.prepare('UPDATE workflow_executions SET updated_at = ? WHERE id = ?').run(now, orig.id);
        ...
        if (gate.status === GATE_STATUS.PENDING) {
            return { success: false, error: 'approval_not_approved: PENDING' };
        }
        if (gate.status === GATE_STATUS.DENIED) {
            return { success: false, error: 'approval_not_approved: ' + (gate.error || 'DENIED') };
        }
        return { success: false, error: 'governance_failed: ' + (gate.error || gate.status) };
    } catch (e) {
        return { success: false, error: 'replay_error: ' + e.message };
    }
}

async function getExecution(db, id, orgId) {
    try {
...
async function listExecutions(db, filters = {}) {
    try {
        const c = [], p = [];
        ...
        const cnt = await db.prepare('SELECT COUNT(*) as total FROM workflow_executions ' + w).get(...p);

...
function authenticate(req, res, next) {
  ...
  if (mode === 'headers') return authHeaders(req, res, next);
  return authJwt(req, res, next);
}

function authHeaders(req, res, next) {
...
function authJwt(req, res, next) {
  ...
  try {
    ...
    return res.status(401).json({ error: 'authentication_failed', detail });
  }
  const userId = payload.sub || payload.user_id || null;
  const orgId = payload.org_id || null;
  if (!userId || !orgId) {
    return res.status(401).json({ error: 'authentication_failed', detail: 'token must contain user and org claims' });
  }
  req.auth = { userId, orgId, mode: 'jwt' };
  next();
}
